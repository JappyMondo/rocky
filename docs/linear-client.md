...
- No real credentials, uploads, sessions, activities or comments were used or created. App-token reads, initial session discovery/recovery remain live gates. Local manual Runs intentionally do not create an Agent Session. No internal `Issue.agentSessions` or `Comment.agentSessions` fields are used; no arbitrary session reuse is added.
- The mirror does not enforce a comment count or wait for automatic comments to appear. Start, terminal and explicit Workflow deliveries retain stable IDs and verified payloads; human replies and automatic question comments do not gate execution.
- Run links use the configured private Tailscale origin or localhost. Attachment, comment, select-body and session-link acceptance/rendering each require live evidence after NG-651 review. The UI stays local; no tunnel/auth workaround was added.
