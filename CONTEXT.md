...
- **Review run**: One execution of Rocky's review against a PR/MR head. States: `queued` (debouncing), `running`, then exactly one of `completed` / `failed` / `superseded` (a newer push replaced it) / `cancelled` (PR closed). There is one review mode: every run sees the full PR diff, with Rocky's open comments passed as prior context ("resolve what the push addressed, don't repeat the rest").
