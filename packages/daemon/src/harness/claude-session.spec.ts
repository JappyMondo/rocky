...
it('routes only the owned native session, preserves the host login, and restores resume aliases', async () => {
  ...
  try {
    ...
    const input = { transcriptPath: join(root, 'run', 'step.jsonl'), env: { HOME: root, CLAUDE_CONFIG_DIR: configDir } };
    const session = await prepareClaudeSession(input, id);
    await routeClaudeSession({ session_id: id, transcript_path: native }, session.route);
    session.verify();
    expect((await lstat(native)).isSymbolicLink()).toBe(true);
    expect(await readFile(session.route.record, 'utf8')).toBe('native record\n');
    ...
    expect(await readFile(credential, 'utf8')).toBe('credential sentinel, never imported by Rocky');
